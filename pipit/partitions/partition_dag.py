...
class Partition_DAG:
    ...
    def merge_strongly_connected_components(self):
        """ Merge strongly connected components into one partition
        """

        components = self._tarjan_strongly_connected()
        for component in components:
            if len(component) > 1:
                # merge the components
                parent_partition_id = component[0]
                for child_partition_id in component[1:]:
                    self.merge_partitions(parent_partition_id, child_partition_id)



    def _tarjan_strongly_connected(self):
        ...
        def strong_connect(partition: Partition):
            ...
            for child in partition.children:
                child_id = child.partition_id
                if child_id not in visited:
                    strong_connect(child)
                    partition.low_link = min(partition.low_link, child.low_link)
                elif child in stack:
                    partition.low_link = min(partition.low_link, self.partition_map[child_id].index)
        ...
        for partition in self.partition_map.values():
            partition._initialize_for_tarjan()

        for partition_id, partition in self.partition_map.items():
