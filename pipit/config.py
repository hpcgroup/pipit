...
def log_level_validator(key, value):
    if value not in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]:
        raise ValueError(
            'Error loading configuration: The Value "{}" for Configuration "{}" must be a valid log level'.format(
                value, key
            )
        )
    else:
...
def url_validator(key, value):
    if value.startswith("http://") or value.startswith("https://"):
        ...
        raise ValueError(
            'Error loading configuration: The Value "{}" for Configuration "{}" must be a valid URL'.format(
                value, key
            )
        )
