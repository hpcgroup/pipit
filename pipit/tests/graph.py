...
root = graph.Node(df.iloc[0]['Name'], None)
...
node = graph.Node(df.iloc[1]['Name'], root)
...
def test_is_empty():
    assert(call_graph.is_empty() == False)
    assert(call_graph.is_empty() != True)
