...
def test_events(otf2_dir):
    ...
    assert len(events_df.loc[events_df['Process'] == 0].loc[events_df['Event Type'] == 'Enter']) == 89

    # 89 Leave events in ping pong trace at process 0 
    assert len(events_df.loc[events_df['Process'] == 0].loc[events_df['Event Type'] == 'Leave']) == 89

    # 60 Enter events in ping pong trace at process 1 
    assert len(events_df.loc[events_df['Process'] == 1].loc[events_df['Event Type'] == 'Enter']) == 60

    # 60 Leave events in ping pong trace at process 1 
    assert len(events_df.loc[events_df['Process'] == 1].loc[events_df['Event Type'] == 'Leave']) == 60
    ...
    mpi_send_df = events_df.loc[events_df["Name"] == "PMPI_Send"].loc[events_df['Event Type'] == 'Enter']
    mpi_recv_df = events_df.loc[events_df["Name"] == "PMPI_Recv"].loc[events_df['Event Type'] == 'Enter']
