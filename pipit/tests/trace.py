...
#     # Ensure we get same result despite using different APIs
#     assert all_equal(
...
#     # TODO: this is not enough for case I texted Abhinav for

#     # validate test
#     keep = trace.filter("Timestamp (ns)", ">", "133 ms", validate="keep").events
#     invalid = trace.filter("Timestamp (ns)", ">", "133 ms", validate=False).events
#     remove = trace.filter("Timestamp (ns)", ">", "133 ms", validate="remove").events

#     # invalid is same as raw dataframe selection
#     assert all_equal(invalid, trace.events[trace.events["Timestamp (ns)"] > 1.33e08])

#     # keep has more rows than invalid
#     # invalid has more rows than remove
#     assert len(keep) > len(invalid) and len(invalid) > len(remove)
...
#     # number of enter/leave rows should match
#     assert len(remove[remove["Event Type"] == "Enter"]) == len(
#         remove[remove["Event Type"] == "Leave"]
#     )
