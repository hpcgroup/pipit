...
class HPCToolkitReader:
	...
	def read(self):
		...
		# iterates through each trace line for the fiven rank and adds it to the dictionary
		...
		data = {'Function Name':[], 'Event Type':[], 'Time':[], 'Process':[], 'Graph_Node': [], 'Level': []}
		min_max_time = experiment_reader.get_min_max_time()


		# cycle through trace headers (each iteration in this outer loop is a seperate process/thread/rank)
...
# if it was then we skip it, as to not have multiple sets of open/close events for a function that it's still in 					
				if(last_id != calling_context_id):
					...
					# we want to close every enter time event higher than than interest node, because those functions have exited
					...
					# the list of nodes higher than interesect_level (the level of interesect_node)
					# all of the nodes in this list have entered into the function since the last poll
					# so we want to create entries in the data for the Enter event
			...
			# after reading through all the trace lines, some Enter events will not have matching Exit events,
			# as the functions were still running in the last poll.
			# Here we are adding Exit events to all of the remaining unmatched Enter events 
