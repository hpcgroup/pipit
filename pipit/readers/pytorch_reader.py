    ...
    def read(self):
        for curr_rank_file in self.files:
            ...
            with(open(self.dir_name + "/" + curr_rank_file, 'r')) as file:
                ...
                temp_df = complete_events_df[["ph", "cat", "name", "pid", "tid", "ts"]].copy()
        ...
        df["ts"] -= min(df["ts"])  # maybe can't do this when we have a multi-gpu trace? (only if there is a global timestamp)
        df["ts"] *= 1000
        df.sort_values(by="ts", ignore_index=True, inplace=True)
        df.rename(columns = {"ph": "Event Type", "name": "Name", "pid": "Process", "tid": "Thread", "ts": "Timestamp (ns)"}, inplace=True)

        # not very performant right now
        attribute_cols = set(df.columns) - set(["Event Type", "Name", "Process", "Thread", "Timestamp (ns)"])
        df["Attributes"] = [{k: v for k, v in x.items() if pd.notnull(v)} for x in df[list(attribute_cols)].to_dict(orient="records")]
        df.drop(columns=attribute_cols, inplace=True)
        df = df[["Timestamp (ns)", "Event Type", "Name", "Thread", "Process", "Attributes"]]
        ...
        definitions_df.rename(columns = {"Name": "Definition Type", "args": "Attributes"}, inplace = True)
        definitions_df = definitions_df[["Definition Type", "Process", "Thread", "Attributes"]]
